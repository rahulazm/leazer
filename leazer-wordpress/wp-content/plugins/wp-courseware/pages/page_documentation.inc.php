...
function WPCW_showPage_Documentation_shortcodes()
{
	...
	<dl class="wpcw_doc_params">
		...
		<dt><?php _e('user_quiz_grade','wp_courseware') ?></dt>
		<dd><?php _e('<em>(Optional)</em> If true, show quiz grade for unit if unit conatins quiz. (It can be <b>true</b> or <b>false</b>. By default, it\'s <b>false</b>).','wp_courseware') ?></dd>
	</dl>
	...
	<dl class="wpcw_doc_params">
		...
		<dt><?php _e('certificate','wp_courseware') ?></dt>
		<dd><?php _e('<em>(Optional)</em> If true, show certificate button if certificates are enabled and course is complete. (It can be <b>true</b> or <b>false</b>. By default, it\'s <b>false</b>).','wp_courseware') ?></dd>
	...
	<dl class="wpcw_doc_examples">
		...
		<dt><?php _e('Example 5: <code>[wpcourse_progress courses="1,2" user_progress="true" user_grade="true" user_quiz_grade="true" certificate="true" /]</code>','wp_courseware') ?></dt>
		<dd><?php _e('Only shows courses with IDs of 1 and 2 if the user is signed to them. If the user is not signed up to any of those courses, then that course is not shown. Their progress and cumulative grade so far for each course is also shown. Quiz grades are shown if quiz is applicable. Certificate button will be displayed.','wp_courseware') ?></dd>
	</dl>
	<?php

	printf('<h2 class="wpcw_doc_header">%s</h2>', __('Course Enrollment Button Shortcode', 'wp_courseware'));
	?>
	<p><?php _e('The <code>[wpcourse_enroll]</code> shortcode will allow you to create a course enrollment button for your courses.', 'wp_courseware'); ?>
	<?php _e('The shortcode will work for both users who are logged and users who are registering. If a user is logged in and clicks the button
				...
				registration page, and upon registration they will be enrolled into the specified course(s).', 'wp_courseware'); ?></p>
	<p><?php _e('Here\'s a summary of the shortcode parameters for <code>[wpcourse_enroll]</code>:','wp_courseware'); ?></p>

	<h3><?php _e('Parameters:', 'wp_courseware'); ?></h3>
	<dl class="wpcw_doc_params">
		<dt><?php _e('courses','wp_courseware') ?></dt>
		<dd><?php _e('<em>(Required)</em> A comma-separated list of course IDs that the user will be enrolled into. If this is not specified, then the button will not appear.','wp_courseware') ?></dd>

		<dt><?php _e('enroll_text','wp_courseware') ?></dt>
		<dd><?php _e('<em>(Optional)</em> This is simply the text that will appear on the button. If the parameter is not specified, "Enroll Now" will be displayed as default.','wp_courseware') ?></dd>
	</dl>

	<br/>
	<h3><?php _e('Here is an example of how <code>[wpcourse_progress]</code> shortcode works:','wp_courseware') ?></h3>
	<dl class="wpcw_doc_examples">
		<dt><?php _e('Example 1: <code>[wpcourse_enroll courses="1,2,5,10" enroll_text="Enroll Today!" /]</code>','wp_courseware') ?></dt>
		<dd><?php _e('Shows a button that will allow a user to enroll into the specified courses. If a logged on user is already enrolled into a course that was specified, they will only be enrolled
						into the courses that apply. If the user is enrolled into all the courses a message will display that they are already enrolled. The actual button will display "Enroll Today!".','wp_courseware') ?></dd>
	</dl>
	<?php
}
