    ...
    public function render_completionBox_complete()
    {
        ...
        if ($this->userProgress->isCourseCompleted()) {
            ...
            if ('use_certs' == $this->parentData->course_opt_use_certificate && $certificateDetails) {
                $certHTML = sprintf(
                        '<div class="wpcw_fe_progress_box_download"><a href="%s" class="fe_btn fe_btn_download">%s</a></div>', WPCW_certificate_generateLink($certificateDetails->cert_access_key), __('Download Certificate', 'wp_courseware')
                );
            }
