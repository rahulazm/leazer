...
class WPCW_quiz_MultipleChoice extends WPCW_quiz_base
{
    ...
    public function editForm_toString()
    {
        ...
        if ($answerList) {
            ...
            foreach ($answerList as $answerItem) {
                ...
                $html .= sprintf(
                        '<th>%s <span class="wpcw_inner_hint">%s</span></th>', __('Answer Image URL', 'wp_courseware'), __('(Optional) ', 'wp_courseware')
                );
     ...
     */
    public function processAnswersWithRandomOption()
    {
        global $wpcwdb, $wpdb;
        $wpdb->show_errors();
    ...
    protected function getSection_showRandomizeAnswersField($columnCount)
    {
        $html = '<tr>';
        $html .= sprintf(
                '<th>%s<span class="wpcw_inner_hint">%s</span></th>', __('Randomize Answers?', 'wp_courseware'), __('(Optional)', 'wp_courseware')
        );
        ...
        $html .= sprintf(
                '<input name="question_multi_random_enable_%s" class="wpcw_quiz_details_enable" type="checkbox" %s />', $this->quizItem->question_id, ($this->quizItem->question_multi_random_enable > 0 ? 'checked="checked"' : ''), __('Yes, randomize the order of these answers.', 'wp_courseware')
        );
        ...
        $html .= sprintf(
                ...
                // Hide if not enabled
                ($this->quizItem->question_multi_random_enable ? '' : 'style="display: none;"'), __('Number of answers to display:', 'wp_courseware'), $this->quizItem->question_id, $this->quizItem->question_multi_random_count, __('The correct answers will always appear in the selection of answers.', 'wp_courseware')
        );
