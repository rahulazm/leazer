...
function WPCW_courses_renderCourseList($courseID, $options)
{
	...
	foreach ($moduleList as $moduleID => $moduleObj)
	{
		...
		// No toggle section, so extend the row to correctly fill the width.
			...
			$moduleTitleArea = sprintf('<td colspan="3">%s</td>', $moduleObj->module_title);
			...
			if($user_quiz_grade && !$widget_mode && is_user_logged_in())
			{
				...
				{
					$moduleTitleArea = sprintf('<td colspan="3">%s</td>', $moduleObj->module_title);
					$quizGradeArea = sprintf('<td colspan="3">%s</td>',  __('Quiz Grade','wp_courseware'));
				}
			}
		}
			...
			foreach ($units as $unit)
			{
				...
				if ($showUnitLinks)
				{
					...
					if ($userProgress->canUserAccessUnit($unit->ID))
					{
						...
						get_permalink($unit->ID), $unit->post_title,
					...
					else
					{
						...
						$unit->post_title, __('Unit', 'wp_courseware'), $unit->unit_meta->unit_number);
					}
				}
				...
				$colspan, $unit->post_title, __('Unit', 'wp_courseware'), $unit->unit_meta->unit_number);
				}
			}
		} // end show units
