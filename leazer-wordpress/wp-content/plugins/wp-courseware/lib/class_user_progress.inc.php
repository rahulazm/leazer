...
class UserProgress
{
	...
	function courseCompletedDate(){
		...
		if ($this->isCourseCompleted()){
			...
			$last_unit_date = $wpdb->get_var("SELECT unit_completed_date
			FROM $wpcwdb->user_progress
			WHERE unit_id = $last_unit_id AND user_id = $this->userID
			");
	 ...
	 */
	function hasUserCompletedCoursePrerequisites($userID) {
		...
		if ( is_array( $course_prereqs ) ) {
			foreach( $course_prereqs as $prereq_id => $prereq_label ) {
				$course_progress = new UserProgress($prereq_id, $userID);
